...
def generate_qr(request):
    if request.method == 'POST':
        ...
        now = timezone.now()
        try:
            latitude = float(request.POST.get('latitude') or 18.5204)
            longitude = float(request.POST.get('longitude') or 73.8567)
        ...
        except (Subject.DoesNotExist, ClassYear.DoesNotExist, ValueError, TypeError):
            messages.error(request, "Please select a valid Class Year and Subject.")
